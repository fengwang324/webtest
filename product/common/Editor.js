...
{  KS_EditArea.document.open();
   KS_EditArea.document.write('<head><link href="'+InstallDir+'CB_Editor/Editor_Style.CSS" type="text/css" rel="stylesheet"></head>');
	  ...
	  default :
		    switch (NewMode)
		    {
		    	case "CODE":
				  KS_EditArea.document.designMode="On";
				  KS_EditArea.document.open();
				  KS_EditArea.document.write("<head><link href="+InstallDir+"CB_Editor/Editor_Style.CSS type=text/css rel=stylesheet></head><body MONOSPACE>"+sBody);
...
function ShowMouseRightMenu(event,Style)
{
   ...
   if ((Style==1)&&(CurrMode=='EDIT'))
	...
	MenuStr="<TABLE Border=0 cellpadding=0 cellspacing=0 class=Menu width=138><tr><td width=20 bgcolor=#0072BC><img src="+InstallDir+"CB_Editor/images/about.gif><\/td><td>"+MenuStr+"<\/td><\/tr><\/TABLE>"
	ObjPopDocument.open();
	ObjPopDocument.write("<head><link href='"+InstallDir+"CB_Editor/ContextMenu.css' type=\"text/css\" rel=\"stylesheet\"></head><body scroll=\"no\" onConTextMenu=\"event.returnValue=false;\">"+MenuStr);
...
{  var MenuFunRowStr='';
	...
	if (MenuImage!="")
	{
		MenuFunRowStr+="&nbsp;<img border=0 src='"+InstallDir+"CB_Editor/images/"+MenuImage+"' align=absmiddle>&nbsp;";
	}
	else
	{
		MenuFunRowStr+="&nbsp;";
...
function GetMenuRowStr(DisabledStr, MenuOperation, MenuImage, MenuDescripion)
{
	...
	if (MenuImage!="")
	{
		MenuRowStr+="&nbsp;<img border=0 src='"+InstallDir+"CB_Editor/images/"+MenuImage+"' align=absmiddle "+DisabledStr+">&nbsp;";
	}
	else
	{
		MenuRowStr+="&nbsp;";
...
{
	OpenWindow(InstallDir+'CB_Editor/AttributeWindow.asp',360,120,window)
	KS_EditArea.focus();
}
...
function SetEditPic()
{
	if (KS_EditArea.document.selection.type=="Control")
	{
		...
		{    
			selectedImage=KS_EditArea.document.selection.createRange()(0);
			var ReturnValue=OpenWindow('../CB_Editor/InsertPicture.asp?FromUrl=1&ChannelID=1&action=edit&src='+selectedImage.src+'&width='+selectedImage.width+'&height='+selectedImage.height+'&border='+selectedImage.border+'&alt='+encodeURI
(selectedImage.alt)+'&vspace='+selectedImage.vspace+'&hspace='+selectedImage.hspace+'&align='+selectedImage.align,420,370,window);
			if (ReturnValue!='')
			{
...
function FullScreen(InstallDir,Style,ChannelID)
{
	...
	window.open(InstallDir+'CB_Editor/fullscreen.asp?ChannelID='+ChannelID+'&Style='+Style, '','toolbar=no, menubar=no, top=0,left=0,width=1024,height=768, scrollbars=no, resizable=no,location=no, status=no');
}
function TextBGColor()
{
	...
	var ReturnValue=OpenWindow(InstallDir+'CB_Editor/SelectColor.asp',230,190,window);
	if (ReturnValue!=null)
	{
...
{
	var ReturnValue=OpenWindow(InstallDir+'CB_Editor/InsertTable.asp',250,220,window);
	InsertHTMLStr(ReturnValue);
	KS_EditArea.focus();
}
function InsertPage(InstallDir)
{
	var ReturnValue=OpenWindow(InstallDir+'CB_Editor/InsertPage.asp',320,110,window);
	InsertHTMLStr(ReturnValue);
	KS_EditArea.focus();
}
...
{
	KS_EditArea.focus();
	var ReturnValue=OpenWindow(InstallDir+'CB_Editor/InsertMarquee.asp',260,50,window); 
	InsertHTMLStr(ReturnValue);
	KS_EditArea.focus();
}
...
{
	KS_EditArea.focus();
	var ReturnValue=OpenWindow(InstallDir+'CB_Editor/Calculator.asp',160,180,window);
	if (ReturnValue!=null)
	{
...
{
	KS_EditArea.focus();
	var ReturnVlaue =OpenWindow(InstallDir+'CB_Editor/InsertFrame.asp',280,118,window);
	if (ReturnVlaue != null)
	{
...
{
  var arr = OpenWindow(InstallDir+'CB_Editor/Abort.asp',220,100,window);
}
function InsertSymbol(InstallDir)
{
  var ReturnValue = OpenWindow(InstallDir+'CB_Editor/InsertTsfh.asp',300,190,window); 
  if (ReturnValue!='')
  {
    var TempArray=ReturnValue.split("$$$");
    InsertHTMLStr(TempArray[0]);
...
function InsertFileFromUp(FileList,InstallDir)
{
	...
	{     var ext=getFilePic(Files[i]);
          ...
          InsertHTMLStr("<img border=0 src='"+InstallDir+"CB_Editor/images/FileIcon/"+ext+"'> <a href="+Files[i]+" class='newsContent' target='_blank'>[点击浏览该文件:"+file+"]</a>"+br);	
     }
}
// 按文件扩展名取图，并产生链接
...
{		
	var ReturnValue=OpenWindow(InstallDir+'CB_Editor/InsertPicture.asp?FromUrl='+FromUrl+'&ChannelID='+ChannelID,420,370,window);
	if (ReturnValue!='')
	{
		var TempArray=ReturnValue.split("$$$");
		InsertHTMLStr(TempArray[0]);
...
{ 
  var ReturnValue = OpenWindow(InstallDir+'CB_Editor/InsertFlash.asp?FromUrl='+FromUrl+'&ChannelID='+ChannelID,400,320,window); 
  if (ReturnValue!='')
  {
    var TempArray=ReturnValue.split("$$$");
    InsertHTMLStr(TempArray[0]);
...
{
  var ReturnValue=OpenWindow(InstallDir+'CB_Editor/InsertVideo.asp?FromUrl='+FromUrl+'&ChannelID='+ChannelID,400,320,window);
  if (ReturnValue!='')
  {
    var TempArray=ReturnValue.split("$$$");
    InsertHTMLStr(TempArray[0]);
...
{
  var ReturnValue=OpenWindow(InstallDir+'CB_Editor/InsertRM.asp?FromUrl='+FromUrl+'&ChannelID='+ChannelID,400,320,window);
  if (ReturnValue!='')
  {
    var TempArray=ReturnValue.split("$$$");
    InsertHTMLStr(TempArray[0]);
...
{
  var ReturnValue=OpenWindow(InstallDir+'CB_Editor/InsertUpFile.asp?FromUrl='+FromUrl+'&ChannelID='+ChannelID,400,120,window);
  if (ReturnValue!='')
  {    var TempArray=ReturnValue.split("$$$");
    InsertHTMLStr(TempArray[0]);
  }  KS_EditArea.focus();
...
{
	KS_EditArea.focus();
	var ReturnValue = OpenWindow(InstallDir+'CB_Editor/InsertSpecialHR.asp',320,120,window); 
...
function PicAndTextArrange(InstallDir)
{
	if(ImageSelected())
	{
		...
		var ReturnValue=OpenWindow(InstallDir+'CB_Editor/SelectPicStyle.asp',380,130,window);
		if(ReturnValue)
		{
...
{
  var Temp=window.showModalDialog(InstallDir+"CB_Editor/InsertSearch.asp", window, "dialogWidth:340px; dialogHeight:190px; help: no; scroll: no; status: no");
}
